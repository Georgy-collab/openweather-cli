...
def get_current_weather(city : str = None, lantitude: float = None, longitude: float = None) -> dict | None:
    if city:
        lantitude, longitude = get_coordinates(city)
        if lantitude is None or longitude is None:
            print(f"Не удалось получить координаты города {city}")
            return None
        return get_weather_by_coordinates(lantitude, longitude)
    elif lantitude and longitude:
        return get_weather_by_coordinates(lantitude, longitude)
    else:
...
def get_weather_by_coordinates(lantitude: float, longitude: float) -> dict | None:
    url = f"https://api.openweathermap.org/data/2.5/weather?lat={lantitude}&lon={longitude}&appid={API_KEY}&units=metric&lang=ru"
    response = get_request(url)
    if response is None:
        return None
    data = response.json()
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                weather = get_current_weather(lantitude=lat, longitude=lon)
                if weather:
